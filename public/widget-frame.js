...
var PostMessage = {
  ...
  onMessage: function (event) {
    ...
    if (data.event == 'visible') {
      ...
      triggerEvent(window, 'tg:inforequest', {
        detail: {
...
window.TWidgetAuth = {
  ...
  logIn: function () {
    ...
    Telegram.Login.auth({ bot_id: options.bot_id, lang: 'en' }, function (user) {
      if (user) {
        xhrRequest(telegramOrigin + '/auth', user, function (err, result) {
          if (result.ok) {
...
(function () {
  ...
  var TWidgetLogin = {
    ...
    auth: function () {
      ...
      TWidgetLogin.activePopup = window.open(telegramOrigin + '/auth?bot_id=' + TWidgetLogin.botId + (TWidgetLogin.paramsEncoded ? '&' + TWidgetLogin.paramsEncoded : ''), 'telegram_oauth', 'width=' + width + ',height=' + height + ',left=' + left + ',top=' + top + ',status=0,location=0,menubar=0,toolbar=0');
    ...
    getAuth: function (init) {
      var xhr = getXHR();
      xhr.open('POST', telegramOrigin + '/auth/get?bot_id=' + TWidgetLogin.botId + (TWidgetLogin.lang ? '&lang=' + encodeURIComponent(TWidgetLogin.lang) : ''));
      xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
      xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
